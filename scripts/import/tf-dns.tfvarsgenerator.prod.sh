...
# Now, for each zone in "prod_zone_list.txt", output the Azure zone list in json format and process the file
...
do
        ...
        # Create a write into $dns_zone.tfvars
        if [ -e $dns_zone.tfvars ]
        then
        rm $dns_zone.tfvars
        fi
        echo "recordsets = [" > $dns_zone.tfvars

        # Iterate this file and buile the $dns_zone.tfvars file
            ...
            while IFS='|' read key value; do
                ...
                case "$which_key" in
                    ...
                    then
                        ...
                        type='"'AAAA'"'
                        # Write out to $dns_zone.tfvars file
                        echo "{" \
                        "name = "$name"," \
                        "records = ["$records"]," \
                        "ttl = "$ttl"," \
                        "type = "$type \
                        ${close_item} >> $dns_zone.tfvars
        ...
        # Create a write into $dns_zone.tfvars
        echo "]" >> $dns_zone.tfvars

        # Pretty print the $dns_zone.tfvars file
        # jq . $dns_zone.tfvars

        # Copy to the environment directory
        cp $dns_zone.tfvars ../../environments/prod
