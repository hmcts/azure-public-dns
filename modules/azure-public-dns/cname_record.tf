locals {
  ...
  shutter_all_cname = local.shutter_all == true ? true : (var.zone_platform == "cft" && local.shutter_all_cft == true) ? true : (var.zone_platform == "sds" && local.shutter_all_sds == true) ? true : lookup(yamldecode(var.shutter_config), "shutter_all_cname", false)
